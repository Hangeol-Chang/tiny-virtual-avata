// Runner for the transparent avatar-overlay scene. Adapted from FaceAvatarRunner, but
// skips the debug bounding-box annotation entirely and additionally feeds a
// FaceKeypointsVisualizer so raw face keypoints can be toggled on for debugging.
...
{
  public class AvatarOverlayRunner : VisionTaskApiRunner<FaceDetector>
  {
    [SerializeField] private FaceAvatarController _faceAvatarController;
    [SerializeField] private FaceKeypointsVisualizer _faceKeypointsVisualizer;
    ...
    protected override IEnumerator Run()
    {
      ...
      _faceKeypointsVisualizer.imageSize = imageSize;
      _faceKeypointsVisualizer.isMirrored = false;
      _faceKeypointsVisualizer.rotationAngle = RotationAngle.Rotation0;
        ...
        switch (taskApi.runningMode)
        {
          case Tasks.Vision.Core.RunningMode.IMAGE:
            if (taskApi.TryDetect(image, imageProcessingOptions, ref result))
            {
              ...
              _faceKeypointsVisualizer.DrawNow(result);
            }
            else
            {
              _faceAvatarController.DrawNow(default);
              _faceKeypointsVisualizer.DrawNow(default);
            }
            break;
    ...
    private void OnFaceDetectionsOutput(FaceDetectionResult result, Image image, long timestamp)
    {
      ...
      _faceKeypointsVisualizer.DrawLater(result);
    }
  }
}
